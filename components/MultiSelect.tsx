...
// БЕЗОПАСНАЯ ФИЛЬТРАЦИЯ
  ...
  return (
    <div className="relative flex flex-col min-w-[160px]" ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 z-[100] w-full mt-2 bg-white border border-gray-200 rounded-2xl shadow-2xl p-2 animate-in fade-in zoom-in duration-150">
          <input 
            type="text" 
            placeholder="Поиск..." 
            value={search} 
          ...
          <div id={listboxId} role="listbox" aria-multiselectable="true" className="max-h-48 overflow-y-auto space-y-1">
            ...
            {filteredOptions.length === 0 ? (
              ...
              filteredOptions.map(opt => (
                <label key={String(opt)} className="flex items-center px-2 py-2 hover:bg-gray-50 rounded-xl cursor-pointer group">
                  <input 
                    type="checkbox" 
                    className="w-4 h-4 rounded border-gray-300 text-teal-600" 
                    checked={value.includes(String(opt))} 
                    onChange={() => {
                      const valStr = String(opt);
                      const next = value.includes(valStr) ? value.filter(i => i !== valStr) : [...value, valStr];
                      onChange(next);
                    }} 
                  />
                  <span className="ml-2 text-xs text-gray-600 truncate group-hover:text-gray-900">{String(opt)}</span>
                </label>
              ))
            )}
          </div>
