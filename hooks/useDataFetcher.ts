...
const normalizeStringArray = (value: unknown): string[] => {
  ...
  if (value && typeof value === 'object') {
    const maybeArrayLike = value as { toArray?: () => unknown[] };
    if (typeof maybeArrayLike.toArray === 'function') {
      return normalizeStringArray(maybeArrayLike.toArray());
    }
...
const parseArrowPayload = (payload: WorkerArrowResult): QueryResult => {
  ...
  return {
    ...
    options: {
      cities: normalizeStringArray(optionsRows[0]?.cities),
      years: normalizeStringArray(optionsRows[0]?.years),
      months: normalizeStringArray(optionsRows[0]?.months),
      formats: normalizeStringArray(optionsRows[0]?.formats),
      vendors: normalizeStringArray(optionsRows[0]?.vendors),
    },
  };
};
